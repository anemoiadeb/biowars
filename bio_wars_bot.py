...
ROLLBACK_CHANCE = 0.005   # 0.5% шанс отката при любом действии
...
def maybe_rollback(u: Dict) -> Optional[str]:
    ...
    if not upgradeable or random.random() > ROLLBACK_CHANCE:
        return None
...
def _buy(fu, key: str) -> Tuple[bool, str, Dict, Optional[str]]:
...
async def cb_bi(cq: CallbackQuery) -> None:
    ...
    if ok:
        txt = fmt_shop(u)
        if rb: txt = rb + "\n\n" + txt
        await cq.message.edit_text(txt, reply_markup=kb_shop(u)); await cq.answer(f"🦫 {t}!")
...
async def cb_bl(cq: CallbackQuery) -> None:
    ...
    if ok:
        txt = fmt_shop(u)
        if rb: txt = rb + "\n\n" + txt
        await cq.message.edit_text(txt, reply_markup=kb_shop(u)); await cq.answer(f"\u2620\ufe0f {t}!")
    else:  await cq.answer(f"\u274c {t}", show_alert=True)
